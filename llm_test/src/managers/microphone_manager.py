...
class MicrophoneManager:
    ...
    def _apply_effective_samplerate(self, samplerate: float, reason: str = "") -> None:
        ...
        rate_changed = abs(previous_rate - samplerate_value) > 1e-3 if previous_rate is not None else True
        ...
        *,
        ...
        for samplerate in candidates:
            try:
                ...
                if update_state:
                    reason = "디바이스 호환성" if abs(samplerate - self.target_sample_rate) > 1e-3 else ""
                ...
                logger.debug(
                    ...
                    int(samplerate) if abs(samplerate - int(samplerate)) < 1e-6 else samplerate,
                    device_index,
                    exc,
                )
    ...
    def _resample_audio(self, audio_bytes: bytes, source_rate: float) -> bytes:
        """장치 샘플레이트와 설정 샘플레이트가 다를 때 오디오를 재샘플링."""
        if not audio_bytes:
            return audio_bytes
